...
| 4 - MEDIUM | Next story needs creation | Create story with Oracle (Athena) -- `/bmad:bmm:agents:oracle` -> CS |
| 5 - NORMAL | `ready-for-dev` story available | Implement with Dev (Amelia) -- `/bmad:bmm:agents:dev` -> DS |
| 6 - LOW | Epic complete, no retrospective | Run retro with Oracle (Athena) -- `/bmad:bmm:agents:oracle` -> RT |
