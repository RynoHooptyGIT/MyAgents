...
- Files modified (group by `backend/`, `packages/frontend/`, `_bmad/`, other)
...
3. Check for new or modified files in `_bmad-output/` (planning artifacts, story files).
...
Apply the Navigator priority decision tree to determine what the next session should focus on:

| Priority | Condition | Recommendation |
|----------|-----------|----------------|
| 1 - CRITICAL | Story stuck `in-progress` > 2 days | Resume with Dev (Amelia) -- `/bmad:bmm:agents:dev` -> DS |
| 2 - CRITICAL | Uncommitted work in working tree | Commit or stash before ending session |
| 3 - HIGH | Story in `review` status | Code review with Dev (Amelia) -- `/bmad:bmm:agents:dev` -> CR |
| 4 - MEDIUM | Next story needs creation | Create story with SM (Bob) -- `/bmad:bmm:agents:sm` -> CS |
| 5 - NORMAL | `ready-for-dev` story available | Implement with Dev (Amelia) -- `/bmad:bmm:agents:dev` -> DS |
| 6 - LOW | Epic complete, no retrospective | Run retro with SM (Bob) -- `/bmad:bmm:agents:sm` -> ER |
| 7 - INFO | No recent health check | Run health check with Custodian (Sentinel) -- `/bmad:bmm:agents:custodian` |
...
---
Generated by: Project Navigator (Navi) - Session Handoff Workflow
```
