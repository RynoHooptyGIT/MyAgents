...
2. **BMAD detection**: If `_bmad/bmm/config.yaml` exists:
   - Search for `_bmad-output/sprint-status.yaml`
...
**If `_bmad-output/sprint-status.yaml` exists AND story context was loaded:**
...
- This regenerates `_bmad-output/context/sprint-digest.md` from the updated sprint-status.yaml
