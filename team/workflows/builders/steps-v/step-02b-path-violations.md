...
**What we're catching:**
- Content like: `Load {project-root}/team/foo/workflows/.../file.csv`
...
if pwd | grep -q "/modules/[^/]\+/" && ! pwd | grep -q "/bmb/"; then
  grep -rn "{project-root}/team/" steps-c/ steps-e/ steps-v/ 2>/dev/null || echo "No bmb-specific paths found"
fi
```
