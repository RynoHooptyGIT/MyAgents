...
> `scripts/word_metrics.py` is not ported. For manual word counts: use `wc -w <file>` for the document total, then either split the file at each heading and re-run `wc -w`, or count words per section by hand. Mark section counts as approximate when estimated. Ground the reduction summary in these numbers.
