...
> `scripts/recon_kit.py` is not ported. Every `uv run scripts/recon_kit.py <subcommand>` invocation in this skill (in `skill.md`, `references/`, and the type packs) is a manual procedure in this fork: run the `slug` step by naming the run folder from `topic` + `type` following `{workflow.run_folder_pattern}`; run `tally` by grepping `.memlog.md` for `--type claim` lines and counting last-wins statuses per `ref=`; run `staleness` by comparing each claim's `pub_date` against the pack's per-class window and flagging expired claims; run `citations` by cross-checking inline `[n]` markers against the appendix and reporting orphans/dangling by hand; run `escape-sources` by HTML-escaping every appendix cell yourself and only linking validated `http(s)` URLs. See `references/process.md` for how these fold into the process flow.
