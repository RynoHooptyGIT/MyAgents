...
- Escalate complex security issues to Oracle for Shield (Security Auditor) deployment
...
Agents communicate through file-based messaging with Oracle as the hub.
...
Agents can request Oracle to spawn specialized helper sub-agents for tasks that exceed their scope or need parallel attention.
...
1. Write a request to team/_memory/_comms/requests/ addressed to Oracle
2. Specify:
...
3. Oracle evaluates the request and spawns the helper if appropriate
...
- Oracle tracks all active helpers and their status

**Helper Agent Template:**
When Oracle spawns a helper, it uses this context:
```
...
- When in doubt, ask Oracle to check with the CEO
...
- When confused, STOP and surface the confusion to the user or Oracle
...
2. If it exists, read it FIRST — it contains your assignments and context from Oracle
3. Follow the assignments in priority order
4. If no mission briefing exists, await instructions from Oracle or the user
